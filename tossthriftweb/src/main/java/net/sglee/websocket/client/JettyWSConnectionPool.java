...
public class JettyWSConnectionPool { //implements Runnable {
	private static final Logger logger = LoggerFactory.getLogger(JettyWSConnectionPool.class);
	
	private class Terminator implements java.lang.Runnable {
		private long waitingTimeToStop=10;
		
		public long getMaxWaitingTimeToStop() {
			return waitingTimeToStop;
		}
		
		public void setMaxWaitingTimeToStop(long _ms) {
		...
		public Terminator(JettyWSConnection _e,long _ms) {
			setClient(_e);
			setMaxWaitingTimeToStop(_ms);
		}

		JettyWSConnection connection=null;
		...
		public void run() {
			...
			while(connection.isConnected()) {
				...
				try {
					Thread.sleep(getMaxWaitingTimeToStop());
...
//	private Thread runningThread=null;
	...
	public synchronized void stop() {
		...
		Iterator<Entry<String,JettyWSConnection>> iterator=this.runningConnections.entrySet().iterator();
...
//				this.deactivateConnection(connection);
	...
	private LinkedBlockingQueue<JettyWSConnection> freeConnections=new LinkedBlockingQueue<JettyWSConnection>();
	private Map<String,JettyWSConnection> runningConnections=new ConcurrentHashMap<String,JettyWSConnection>();
	...
	public JettyWSConnectionPool(
		...
		String connectionId=String.valueOf(currentTime)+_connectionId;
		// create connections
		for(int i=0;i<_connectionSize;i++) {
	        ...
	        JettyWSConnection connection=
					new JettyWSConnection(connectionId,i,client,_uri,request,
							this.getWaitingTimeToStop(),this.getWaitingTimeDuringRunning(),this.getTimeout());
...
//	public boolean setConnectionActive(JettyWSConnection _connection) {//,MessageHandler _bynaryMessageHandler,MessageHandler _textMessageHandler) {
//		if(_connection.isActivated()) {
//			logger.info("connection is already activated");
//			return false;
//		}
//		synchronized(_connection) {
////			_connection.setBynaryMessageHandler(_bynaryMessageHandler);
////			_connection.setTextMessageHandler(_textMessageHandler);
//			_connection.setActivated(true);
//		}
//		logger.info("setConnectionActive execute");
//		// Thread starts
//		//////////////////////////////////////
//		this.executor.execute(_connection);
//		//////////////////////////////////////
//		_connection.sendMessage("a client of id " +_connection.getId() + " is activated");
//		
//		return true;
//	}
	
//	public void deactivateConnection(JettyWSConnection _connection) throws Exception {
//		JettyWSConnection rt=this.runningConnections.remove(_connection.getId());
//		if(rt!=_connection || rt==null) {
//			throw new Exception("Connection closing error, connection id mismatch in runningConnections");
//		}
//		_connection.setActivated(false);
//		_connection.setBynaryMessageHandler(null);
//		_connection.setTextMessageHandler(null);
//		this.putFreeConnection(_connection);
//	}
...
//	public void run() {
//		logger.info("JettyWebSocketClientPool is running");
//
////		synchronized(this) {
